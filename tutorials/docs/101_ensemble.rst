...
.. code-block::
    ...
    ;; step 2: creating a ensemble
    (define ensemble-id (create-ensemble dataset-id))
    (log-info "Creating ensemble from dataset: " ensemble-id)
    ;; the new input data to predict for
    (define input-data {"petal width" 1.75
                        "petal length" 2.45})
    ;; creating a single prediction
    (define prediction-id (create-prediction ensemble-id
...
created an `ensemble` following the steps 0 to 2 in the previous snippet.
...
.. code-block::
    ...
    (define batch-prediction-id (create-batchprediction ensemble-id
                                                        test-dataset-id))
