...
WhizzML: 101 - Using a Fusion Model
===================================
...
.. code-block::
    ...
    ;; step 2: creating a fusion model
    (define fusion-id (create-fusion
                        {"models" [{"id" model-id "weight" 1}
                                   {"id" logisticregression-id "weight" 1}]}))
    (log-info "Creating fusion from model and logisticregression: " fusion-id)
...
created a `fusion` following the steps 0 to 2 in the previous snippet.
...
.. code-block::
    ...
    (define batch-prediction-id (create-batchprediction fusion-id test-dataset-id))
